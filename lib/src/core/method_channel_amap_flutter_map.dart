...
import 'package:x_map/src/core/amap_flutter_platform.dart';
import 'package:x_map/src/types/types.dart';
...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  //用户是否在责任区
  Future<bool?> isInDuty(
      {required int mapId, required double lat, required double lng}) {
    return channel(mapId).invokeMethod<bool>(
        'map#isInDuty', <String, dynamic>{'lat': lat, 'lng': lng});
  }

  /// 开始轨迹动画
  Future<void> startLocusAnimation({required int mapId}) {
    return channel(mapId).invokeMethod<void>('polylines#locusAnimation');
  }
}
