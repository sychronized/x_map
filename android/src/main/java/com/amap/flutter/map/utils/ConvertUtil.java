...
//                e.printStackTrace();
    ...
    public static void interpretAMapOptions(Object o, @NonNull AMapOptionsSink sink) {
        try {
            ...
            final Object walkRoute = data.get("walkRoute");
            if (null != walkRoute) {
                Log.e("MapController", "interpretAMapOptions walkRoute: " + walkRoute);
                sink.setWalkRoute(toString(walkRoute));
            }

            final Object areaLevel = data.get("areaLevel");
            if (null != areaLevel) {
                Log.e("MapController", "interpretAMapOptions areaLevel: " + areaLevel);
                sink.setAreaLevel(toString(areaLevel));
            }

            final Object areaCode = data.get("areaCode");
            if (null != areaCode) {
                Log.e("MapController", "interpretAMapOptions setAreaCode: " + areaCode);
                sink.setAreaCode(toString(areaCode));
            }

            final Object tileEnabled = data.get("tileEnabled");
            if (null != tileEnabled) {
                Log.e("MapController", "interpretAMapOptions setTileEnabled: " + tileEnabled);
                sink.setTileEnabled(toBoolean(tileEnabled));
            }
